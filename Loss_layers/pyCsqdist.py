...
class ChiLossLayer(caffe.Layer):
    """
	...
	chidiv = (gt*gt)/(np.maximum(pmap,epsilon))
	chiloss = (np.sum(chidiv)/(bottom[0].num)) #averaged per image in the batch
	chiloss = chiloss - 1
	...
	for hind in range(pmap.shape[2]):
		for wind in range(pmap.shape[3]):
			...
			self.diff[:,:,hind,wind] = const * ( np.sum(np.sum(chidiv*(1-pmap)*iequalk,axis=3),axis=2) - pmap[:,:,hind,wind]*np.sum(np.sum(chidiv*inotequalk,axis=3),axis=2) )
