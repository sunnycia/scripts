...
__min_duration__ = 0.4           # if a shot has length less than this, merge it with others
__absolute_threshold__ = 0.9   # any transition must be no less than this threshold
...
class shotDetector:
    ...
    def run(self, video_path=None):
        ...
        # print(len(hists), hists[0], len(hists[0]), len(hists[2]));exit()
        # compute hist chisquare distances
        scores = [cv2.compareHist(pair[0],pair[1],cv2.HISTCMP_CHISQR) \
        ...
        threshold = __absolute_threshold__

        # print(scores, np.array(scores).max());exit()
